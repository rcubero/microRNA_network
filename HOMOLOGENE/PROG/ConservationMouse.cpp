...
int main(int argc, char *argv[])
{
	...
	// Open output file
	ofstream OUT("DATA/ConservationMouseGene.txt");
	...
	cout << gp << endl;
		...
		ifstream HOM("DATA/homologene_Feb2014.data");
		...
		if(!mouseE.empty())
		{
			...
			for(int i=0;i<mouseE.size();i++)	
			{
			OUT << mouseE[i] << "\t";

				// Conservation Score 
				int count=0;
				if(humanE.size()>0) count++;
				...
				for(int j=0;j<humanE.size();j++) OUT << humanE[j] << " ";
				OUT << "\t";		
